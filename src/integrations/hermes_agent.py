...
def call_hermes(*, task: str, context: Mapping[str, Any], role: str = "specialist") -> HermesResult:
    key = os.getenv("HERMES_API_KEY")
    if not key:
        return HermesResult(status="not_configured", error="HERMES_API_KEY is not configured")
    ...
    try:
        ...
        return HermesResult(status="error", error=str(exc))
